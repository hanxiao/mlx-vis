...
def animate(snapshots, labels=None, timestamps=None, method_name="",
    ...
    """Render animation video from embedding snapshots.

    Args:
        snapshots: list of (n, 2) arrays, one per epoch/iteration.
        labels: optional int array for coloring by class.
        timestamps: optional list of elapsed seconds per snapshot.
        method_name: shown in title (e.g. "umap-mlx").
        dataset_name: shown in title (e.g. "Fashion-MNIST 70K").
        fps: frames per second.
        theme: "dark" or "light".
        colors: custom RGBA array or colormap name.
        point_size: scatter marker size.
        alpha: point transparency.
        init_hold: seconds to hold on first frame.
        end_hold: seconds to hold on last frame.
        save: output mp4 path (required).
    ...
    n_epochs = n_snap - 1  # first snapshot is init
